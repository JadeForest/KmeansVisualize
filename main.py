...
from data import get_sample_data, generate_random_data, generate_grid_data, Kmeans
...
class App(Tk):
    ...
    def widgets_init(self):
        ...
        self.data_select.add_command(
            label="Use default data (from data.json)", command=self.to_default
        )
        self.data_select.add_command(
            label="Use randomly generated data", command=self.to_random
        )
        self.data_select.add_command(label="Use random grid data", command=self.to_grid)
        ...
        self.legend = Canvas(self.fm1, width=150, height=80, background="white")
        self.legend.create_rectangle(
            37 - 4, 40 - 4, 37 + 4, 40 + 4, width=0, fill="red"
        )
        ...
        self.btn = Button(
            self.fm2, text="Run", width=10, height=1, command=self.run_calc
        )
        self.btn.pack(side=LEFT)
        self.checkbtn = Checkbutton(
            self.fm2,
            text="Use Manhattan Distance",
            height=2,
            variable=self.use_manhattan,
        )
    ...
    def run_calc(self):
        ...
        try:
            ...
            messagebox.showerror(
                title="Input Error", message="Positive integer expected for K."
            )
            return
    ...
    def calc_and_draw(self, K):
        ...
        dist_func = "manhattan" if self.use_manhattan else "eculidean"
        for labels, centers in Kmeans(K, self.data, dist_func):
    ...
    def draw_points(self, labels=None):
        ...
        for i in range(len(self.data)):
            x, y = self.data[i]
            self.fig.create_oval(
                x - 3, y - 3, x + 3, y + 3, width=0, fill=COLOR_LIST[labels[i]]
            )
    ...
    def disable_btn(self, disabled: bool):
        ...
        self.checkbtn.config(state=state)
